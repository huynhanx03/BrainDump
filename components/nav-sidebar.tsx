...
function TreeItem({ item, activePath, depth = 0 }: { item: DirectoryItem, activePath: string, depth?: number }) {
    ...
    return (
      <div className="space-y-1">
        <button
          ...
          className="w-full flex items-center py-1.5 px-2 text-[11px] font-black uppercase tracking-widest text-muted-foreground hover:text-primary transition-colors group"
        >
  ...
  return (
    <Link
      href={`/${item.slug}`}
      className={cn(
        "flex items-center justify-between py-1 px-3 text-[13px] font-medium transition-all group rounded-sm",
        isActive
